...
func test_map_dimensions_from_grid():
	...
	var expected_h: float = GameConfig.MAP_GRID_HEIGHT * GameConfig.GRID_SIZE
	assert_almost_eq(GameConfig.MAP_PIXEL_WIDTH, expected_w, 0.01, "MAP_PIXEL_WIDTH 应等于 grid_width * grid_size")
	assert_almost_eq(GameConfig.MAP_PIXEL_HEIGHT, expected_h, 0.01, "MAP_PIXEL_HEIGHT 应等于 grid_height * grid_size")
