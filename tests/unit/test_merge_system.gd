...
func test_different_ids_do_not_cross_merge() -> void:
	GameData.bag.append({id = "rifle", type = "weapon", level = 1})
	GameData.bag.append({id = "rifle", type = "weapon", level = 1})
	GameData.bag.append({id = "laser", type = "weapon", level = 1})
	GameData._check_merge("rifle", 1)
	# rifle只有2个，不合成
	assert_eq(GameData.bag.size(), 3)
	var rifle_count: int = 0
	for item in GameData.bag:
		if item.id == "rifle" and item.level == 1:
			rifle_count += 1
	assert_eq(rifle_count, 2)
...
func test_merge_emits_signal() -> void:
	...
	assert_eq(merged_events[0].id, "rifle")
