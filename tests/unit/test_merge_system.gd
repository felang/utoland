...
func test_merge_three_lv1_towers_to_lv2() -> void:
	...
	InventoryManager.deployed_towers.append({id = "pea_shooter", level = 1, grid_pos = Vector2i(2, 0), deploy_id = 3})
...
func test_no_merge_at_max_level() -> void:
	...
	# lv3 不触发合成
	assert_eq(InventoryManager.deployed_weapons.size(), 3)
...
func test_tower_merge_keeps_position() -> void:
	...
	InventoryManager.deployed_towers.append({id = "pea_shooter", level = 1, grid_pos = Vector2i(5, 5), deploy_id = 12})
	InventoryManager._check_merge("pea_shooter", 1)
	assert_eq(InventoryManager.deployed_towers.size(), 1)
	assert_eq(InventoryManager.deployed_towers[0].level, 2)
