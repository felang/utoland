...
func test_add_exp_no_level_up_below_threshold() -> void:
	...
	PlayerProgression.current_exp = 0
	var threshold: int = PlayerProgression.exp_for_level(2)
	PlayerProgression.add_exp(threshold - 1)
	assert_eq(PlayerProgression.player_level, 1)

func test_add_exp_multi_level_up() -> void:
	PlayerProgression.player_level = 1
	PlayerProgression.current_exp = 0
	var t2: int = PlayerProgression.exp_for_level(2)
	var t3: int = PlayerProgression.exp_for_level(3)
	PlayerProgression.add_exp(t2 + t3)
	assert_gte(PlayerProgression.player_level, 3)
