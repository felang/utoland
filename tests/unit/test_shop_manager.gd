...
func test_refresh_shop_generates_4_slots() -> void:
	_shop.refresh_shop()
	assert_eq(InventoryManager.shop_slots.size(), 4, "刷新后应有 4 个商店槽位")

func test_refresh_shop_slots_have_required_fields() -> void:
	_shop.refresh_shop()
	for slot in InventoryManager.shop_slots:
		if slot.is_empty():
			continue
...
func test_first_shop_has_recommended_weapon() -> void:
	InventoryManager._recommended_weapon = "bow"
	InventoryManager._recommended_tower = "pea_shooter"
...
func test_first_shop_recommended_weapon_is_first_slot() -> void:
	...
	InventoryManager._recommended_tower = "pea_shooter"
	_shop.refresh_shop(true)
	assert_eq(InventoryManager.shop_slots[0].id, "bow", "首次商店第 0 槽为推荐武器")

func test_first_shop_recommended_tower_is_second_slot() -> void:
	InventoryManager._recommended_weapon = "bow"
	InventoryManager._recommended_tower = "pea_shooter"
	_shop.refresh_shop(true)
	assert_eq(InventoryManager.shop_slots[1].id, "pea_shooter", "首次商店第 1 槽为推荐塔")
...
func test_buy_weapon_deducts_coins() -> void:
	...
	var coins_before: int = InventoryManager.coins
	_shop.buy_weapon(0)
	assert_eq(InventoryManager.coins, coins_before - 3, "购买武器后金币应减少")
...
func test_buy_weapon_adds_to_deployed_weapons() -> void:
	...
	_shop.buy_weapon(0)
	assert_eq(InventoryManager.deployed_weapons.size(), 1, "购买武器后应出现在 deployed_weapons")
...
func test_buy_weapon_clears_slot() -> void:
	...
	_shop.buy_weapon(0)
	assert_true(InventoryManager.shop_slots[0].is_empty(), "购买后槽位应被清空")
...
func test_buy_weapon_fails_insufficient_coins() -> void:
	...
	assert_false(InventoryManager.shop_slots[0].is_empty(), "购买失败后槽位应保留")
...
func test_manual_refresh_costs_2_coins() -> void:
	...
	assert_eq(InventoryManager.coins, 50 - config.refresh_cost, "手动刷新应扣除 refresh_cost 金币")
...
func test_manual_refresh_fails_insufficient_coins() -> void:
	...
	var config: ShopConfig = GameConfig.shop_config
	# 确保金币不足 refresh_cost
	InventoryManager.coins = config.refresh_cost - 1
	_shop.refresh_shop()
	var result: bool = _shop.manual_refresh()
...
func test_manual_refresh_does_not_deduct_on_failure() -> void:
	...
	var coins_before: int = InventoryManager.coins
	_shop.manual_refresh()
	assert_eq(InventoryManager.coins, coins_before, "手动刷新失败时不应扣除金币")

func test_manual_refresh_generates_new_slots() -> void:
	InventoryManager.coins = 50
	_shop.refresh_shop()
	_shop.manual_refresh()
	assert_eq(InventoryManager.shop_slots.size(), 4, "手动刷新后应有 4 个槽位")
