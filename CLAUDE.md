...
- **GameData** (`scripts/core/game_data.gd`) — 运行时游戏状态，存储角色属性、金币、波次、已装备武器（`deployed_weapons`）、已布置塔（`deployed_towers`，含 `deploy_id` 稳定标识符）、商店栏位（`shop_slots`）、人口等级（`player_level` 1-7）。`reset()` 初始化新游戏状态。提供 `buy_and_equip_weapon()`、`buy_and_place_tower()` → 购买即部署、`can_buy_item(item_id, level)` → 智能人口判断（考虑合成释放人口）、`move_tower(deploy_id, new_grid_pos) -> bool`、`sell_from_deployed_weapon/deployed_tower(deploy_id)`、`buy_level_up()`、`_check_merge()` 等方法。跨场景传递数据。
...
- **EventBus** (`scripts/core/event_bus.gd`) — 全局事件总线，用于跨系统解耦通信。商店信号：`item_purchased/item_sold/item_merged/player_level_changed/tower_moved`。战斗信号：`wave_started/wave_completed/wave_transition_ready/enemy_killed/boss_killed/coins_changed/coins_generated`。
...
> **商店与战斗融合**: 不再有独立的商店场景。main 场景通过 `Phase` 状态机（SHOP/BATTLE）管理两个阶段。SHOP 阶段底部面板（`ShopOverlay`，CanvasLayer layer=10）采用卡片式 UI 两行布局：上行信息栏（金币/等级/人口/波次），下行操作栏（4 张物品卡片 + 刷新/升级按钮 + 回收区 + 开战按钮）。卡片展示图标+名称+价格。购买武器立即装备到角色（围绕旋转可见），购买塔进入拖拽放置模式（放置成功后扣金币）。拖拽已部署的塔或武器到回收区可卖出。人口满时能触发合成的物品仍可购买（`can_buy_item` 智能判断）。开战后面板滑出隐藏，波次结束后滑回。
