...
**shape 说明：**
- `LINE`：从 start 到 end 的直线（Bresenham 光栅化 + 宽度扩展）
- `RECT`：矩形区域（用 center + size），可实心填充或空心（仅边框）
- `RING`：空心矩形环（用 center + size + width），仅渲染宽度为 width 的边框

注意：移除 ARC 形状，用多条 LINE 段近似弧形效果。ARC 的光栅化规则复杂且难以精确控制，不值得引入。
...
渲染模板时，所有计算出的格子坐标会裁剪到战术区范围内（x=3~36, y=3~20）。宽度扩展后超出范围的格子直接丢弃，不报错。

### 玩家出生点安全

模板渲染完成后，检查玩家出生点 (19, 11) 及其周围 3×3 区域是否全为 GROUND。如果被主干地形侵占，将该区域强制恢复为 GROUND。模板设计时应避免在中心区（x=14~25, y=9~14）放置主干地形。
...
| vertical_canyon | 纵向峡谷 | 垂直 CORRIDOR（两侧 WALL，中间通道 width=3） | 东西分区，走廊射击通道 |
| cross_corridor | 十字走廊 | 十字形 CORRIDOR（水平+垂直交叉） | 四象限独立，交叉点核心 |
| ring_moat | 环形水道 | RING 形 RIVER 围绕中心（width=1，4 缺口） | 中心被水包围，桥是隘口 |
...
Water 的 type3 图片为 2048×192（含 8 帧动画），初期只取前 256×192 作为静态水面。后续可配置 TileSet 动画帧实现水面动效。
...
### 渲染架构：单层 Terrain + 底层铺草

`set_cells_terrain_connect()` 只看同一 TileMapLayer 内的邻居。如果把 grass 和 water 放在不同层，跨层边界无法产生正确的过渡 tile。

**解决方案：**
- **Ground 层**：全部铺满 grass tile（包括 WALL/ABYSS 下方），保证没有视觉空洞
- **Terrain 层**：所有非 GROUND 类型（WALL、ABYSS、BORDER）都放在这一层，用同一个 terrain_set 的 `set_cells_terrain_connect()` 渲染。Water 和 Wall 作为同层内的不同 terrain，引擎能正确计算彼此的过渡 tile

这样 Water/Wall 的 type3 边缘 tile（带透明度）下方会露出 Ground 层的草地，视觉上形成自然过渡。
...
func apply_to_tilemap(map_scene: Node, layout: MapLayout, config: MapGeneratorConfig) -> void:
    ...
    # Terrain 层：WALL + ABYSS + BORDER 放同一层，引擎自动计算彼此过渡
...
- total_prefab_count: Vector2i = Vector2i(8, 15)  # Prefab 总数范围(min, max)，连通性失败时每次减 2 重试
...
- empty_chance / min_prefabs_per_block / max_prefabs_per_block（改用 total_prefab_count）

保留字段：
- prefabs, spawns_per_edge, corner_spawn_chance, min_spawn_spacing
