...
1. 点击武器卡片
2. `ShopManager.buy_weapon(slot_index)` → 验证金币 + `can_buy_item()` → 扣金币
...
4. 成功放置：
   - `ShopManager.confirm_tower_purchase(slot_index, grid_pos)` → 扣金币
   - `DragManager.spawn_tower_node()` 生成塔节点
   - 触发合成检查 + `_handle_merge_tower_cleanup()` 清理被合成的塔节点
   - 卡片标记为已售出
5. 取消放置：卡片恢复可点击状态，不扣金币
...
1. 商店阶段，点击角色身上的武器开始拖拽
2. `DragManager` 新增 `start_weapon_drag(weapon_index)` 流程
3. 拖到回收区 → `GameData.sell_from_deployed_weapon(index)` → 返还金币，`WeaponManager` 移除武器节点
4. 拖到其他地方 → 取消，武器回原位
...
卡片禁用判断从 `can_deploy()` 改为 `can_buy_item(slot.id, 1)`。

## 清理项

1. **删除 `BagRow`** — 从 `shop_overlay.tscn` 移除废弃节点
2. **删除 `_debug_spawn_towers()`** — `main.gd` 中的调试代码
3. **实现 `_handle_merge_weapon_cleanup()`** — 从 no-op stub 改为调用 `WeaponManager` 更新武器节点
4. **清理废弃信号** — `EventBus` 中 `item_deployed` 和 `item_undeployed` 未使用，考虑移除
...
| `scripts/systems/drag_manager.gd` | 新增：武器拖拽、回收区检测逻辑 |
