...
func refresh_shop(is_first: bool = false) -> void:
	...
	InventoryManager.shop_slots = []
	var guaranteed_ids: Array[String] = []
	if is_first:
		if InventoryManager._recommended_weapon != "" and _has_item(InventoryManager._recommended_weapon):
			guaranteed_ids.append(InventoryManager._recommended_weapon)
		if InventoryManager._recommended_tower != "" and _has_item(InventoryManager._recommended_tower):
			guaranteed_ids.append(InventoryManager._recommended_tower)
	for i in config.slot_count:
		if i < guaranteed_ids.size():
			var item_id: String = guaranteed_ids[i]
			InventoryManager.shop_slots.append({
				id = item_id,
				type = _get_item_type(item_id),
				cost = config.item_cost,
			})
		else:
			InventoryManager.shop_slots.append(_generate_random_slot())

func manual_refresh() -> bool:
	var config: ShopConfig = GameConfig.shop_config
	if InventoryManager.coins < config.refresh_cost:
		return false
	InventoryManager.coins -= config.refresh_cost
	EventBus.coins_changed.emit(-config.refresh_cost, InventoryManager.coins)
	refresh_shop()
	return true
...
func buy_weapon(slot_index: int) -> bool:
	...
	if slot.is_empty() or slot.type != "weapon":
		return false
	var success: bool = InventoryManager.buy_and_equip_weapon(slot.id, slot.cost)
	if success:
		InventoryManager.shop_slots[slot_index] = {}
	return success
...
func confirm_tower_purchase(slot_index: int, grid_pos: Vector2i) -> int:
	...
	if slot.is_empty():
		return 0
	var deploy_id: int = InventoryManager.buy_and_place_tower(slot.id, slot.cost, grid_pos)
	if deploy_id > 0:
		InventoryManager.shop_slots[slot_index] = {}
	return deploy_id

func _validate_slot(slot_index: int) -> Dictionary:
	if slot_index < 0 or slot_index >= InventoryManager.shop_slots.size():
		return {}
	var slot: Dictionary = InventoryManager.shop_slots[slot_index]
	if slot.is_empty():
		return {}
	if InventoryManager.coins < slot.cost:
		return {}
	if not InventoryManager.can_buy_item(slot.id, 1):
		return {}
	return slot
